...
# evaluating correct function: simple wall in middle
assets_configs = [ 
"""
spacing: 0.02
plane: "xz"
...
# evaluating correct function: hole somewhere
# assets_configs=[
# """
# spacing: 0.02
# plane: "xz"
...
def save_scene_PMAF(assets):
		filepath = Path(get_package_share_directory('percept_core')) / "auto_generated_scene.yaml"
...
def save_scene(assets):
		filepath = Path(get_package_share_directory('percept_core')) / "auto_generated_scene.npy"
		import numpy as np
		positions = [item['position'] for item in assets]
		np.save(filepath, np.array(positions, dtype=np.float32))
		print(f"Scene saved to {filepath}")
...
def generate_narrow_passage(args):
		...
		print(f'num_points= {len(merged_assets)}')
