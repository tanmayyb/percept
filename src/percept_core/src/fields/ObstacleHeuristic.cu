...
namespace obstacle_heuristic 
{
    ...
    double3 point_dist_vec_normalized) 
  {
    ...
    double3 nn_point_position = make_double3(d_points_x[nn_point_idx], d_points_y[nn_point_idx], d_points_z[nn_point_idx]);

    double3 obstacle_vec = nn_point_position - point_position;

    double3 current_vec = point_dist_vec_normalized * dot(point_dist_vec_normalized, obstacle_vec) - obstacle_vec;     
  ...
  __global__ void kernel(
    ...
    double k_force) 
    {
      ...
      double3 point_rvel_vec = agent_velocity; // Assumed static point velocity
      ...
      if (!(dot(point_dist_vec_normalized, goal_vec_normalized) < -1e-5 && dot(point_dist_vec, point_rvel_vec) < -1e-5)) 
      {
        ...
        if (dist_to_point < detect_shell_rad && norm(point_rvel_vec) > 1e-10) 
        {
          double3 rot_vec = calculate_rotation_vector(
            ...
            nn_index, point_position, point_dist_vec_normalized
          );
...
extern "C" double3 obstacle_heuristic_kernel(
  double* d_points_x, double* d_points_y, double* d_points_z,
  ...
  double max_allowable_force, bool debug, cudaStream_t stream) 
{
  return obstacle_heuristic::launch_kernel(
