...
#include <ga_circular_fields_planner/VelocityHeuristicForce.hpp>
#include <sackmesser/Callbacks.hpp>

namespace ga_circular_fields_planner
{

    VelocityHeuristicForce::VelocityHeuristicForce(const sackmesser::Interface::Ptr &interface, const std::string &name) : interface_(interface)
    {
        config_ = interface->getConfigurations()->load<Configuration>(name + "/velocity_heuristic_force/");
    }

    VelocityHeuristicForce::~VelocityHeuristicForce() = default;

    gafro::Wrench<double> VelocityHeuristicForce::computeForce(const Agent::State &state,  //
                                                               const gafro::Motor<double> &target) const
    {
        ...
        if (interface_->getCallbacks()->execute("get_velocity_heuristic_force", response, request))
        {
    ...
    bool VelocityHeuristicForce::Configuration::load(const std::string &ns, const std::shared_ptr<sackmesser::Configurations> &server)
    {
...
REGISTER_CLASS(ga_circular_fields_planner::Force, ga_circular_fields_planner::VelocityHeuristicForce, "velocity_heuristic_force")