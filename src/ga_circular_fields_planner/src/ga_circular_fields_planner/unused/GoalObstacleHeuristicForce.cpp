#include <ga_circular_fields_planner/GoalObstacleHeuristicForce.hpp>
...
namespace ga_circular_fields_planner
{
    ...
    GoalObstacleHeuristicForce::GoalObstacleHeuristicForce(const sackmesser::Interface::Ptr &interface, const std::string &name)
      : interface_(interface)
    {
        config_ = interface->getConfigurations()->load<Configuration>(name + "/goalobstacle_heuristic_force/");
    }

    GoalObstacleHeuristicForce::~GoalObstacleHeuristicForce() = default;

    gafro::Wrench<double> GoalObstacleHeuristicForce::computeForce(const Agent::State &state,  //
                                                                   const gafro::Motor<double> &target) const
    {
        ...
        if (interface_->getCallbacks()->execute("get_goalobstacle_heuristic_force", response, request))
        {
    ...
    bool GoalObstacleHeuristicForce::Configuration::load(const std::string &ns, const std::shared_ptr<sackmesser::Configurations> &server)
    {
...
REGISTER_CLASS(ga_circular_fields_planner::Force, ga_circular_fields_planner::GoalObstacleHeuristicForce, "goalobstacle_heuristic_force")