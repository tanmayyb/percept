...
metadata:
  created_at: ""
  plan_name: "250226_freq_predsteps_prof_r1"
  label: ""
  tags:
    - "narrow_passage"
    - "performance"
  info: "Benchmark performance for the planner"
...
def _generate_procedural_configs(base_workload_config:dict):
    ...
    planning_frequency_list = np.linspace(10, 100, num=10).astype(int).tolist()
    max_prediction_steps_list = np.linspace(10, 100, num=10).astype(int).tolist()

    for planning_frequency in planning_frequency_list:
        for max_prediction_steps in max_prediction_steps_list:
            label = f"freq_{planning_frequency}_steps_{max_prediction_steps}"
            #     # Create agent config
