...
metadata:
  ...
  name: "25-02-23_narrow_passage_agents_ablation"
  description: "Randomize number of agents"
...
def _generate_workloads(workload_configs:dict):
    ...
    for i, workload_config in enumerate(workload_configs):
        workload_name = f"workload_{i}_{list(workload_config['planner_config']['agents'][0]['forces'].keys())[1]}.yaml"
...
def _generate_procedural_configs(base_workload_config:dict):
    ...
    for heuristic in ["obstacle_heuristic_force", "goalobstacle_heuristic_force", "velocity_heuristic_force", "goal_heuristic_force"]:
        agent = yaml.safe_load(PROCEDURAL_AGENT_YAML)
        agent["forces"][heuristic] = {
            "k_gain": 1.0,
            "detect_shell_radius": 5.0
        }
        base_workload_config_copy = deepcopy(base_workload_config)
        base_workload_config_copy["planner_config"]["agents"].append(agent)
        configs.append(base_workload_config_copy)
    return configs
