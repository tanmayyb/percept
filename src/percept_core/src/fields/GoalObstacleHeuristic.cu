...
namespace goalobstacle_heuristic 
{
  ...
  __device__ double3 calculate_rotation_vector(
    ...
    int* nn_index,
    double3 point_position,
    double3 point_dist_vec_normalized,
    double3 goal_vec) 
  {
    int nn_point_idx = nn_index[i];

    double3 nn_point_position = make_double3(
      d_points_x[nn_point_idx], 
      d_points_y[nn_point_idx], 
      d_points_z[nn_point_idx]
    );

    double3 obstacle_vec = nn_point_position - point_position;

    // Current vector is perpendicular to obstacle surface normal and shows in opposite direction of obstacle_vec
    double3 obstacle_current_vec = point_dist_vec_normalized * dot(point_dist_vec_normalized, obstacle_vec) - obstacle_vec;

    // Project goal vector onto plane perpendicular to point_dist_vec
    double3 goal_current_vec = goal_vec - point_dist_vec_normalized * dot(point_dist_vec_normalized, goal_vec);

    double3 current_vec = normalized(goal_current_vec) + normalized(obstacle_current_vec);
    ...
    return normalized(cross(current_vec, point_dist_vec_normalized));
  }
    ...
    double k_force) 
  {
    ...
    double3 point_rvel_vec = agent_velocity; // Assumed static points
    ...
    // Skip obstacle if behind and moving away
    if (!(dot(point_dist_vec_normalized, goal_vec_normalized) < -1e-5 && dot(point_dist_vec, point_rvel_vec) < -1e-5)) 
    {
      ...
      if (dist_to_point < detect_shell_rad && norm(point_rvel_vec) > 1e-10) 
      {
        ...
        double3 rot_vec = calculate_rotation_vector(
          i, d_points_x, d_points_y, d_points_z,
          nn_index, point_position, point_dist_vec_normalized, goal_vec
        );

        double3 point_rvel_vec_normalized = normalized(point_rvel_vec);

        double3 current_vec = normalized(cross(point_dist_vec_normalized, rot_vec));
  ...
  __host__ double3 launch_kernel(
    ...
    size_t num_points, int* nn_index,
    ...
    double max_allowable_force, bool debug, cudaStream_t stream) 
  {
    ...
    kernel<<<num_blocks, threads, shared_mem_size, stream>>>(
      ...
      agent_radius, point_radius, detect_shell_rad, k_force
    );
...
extern "C" double3 goalobstacle_heuristic_kernel(
  double* d_points_x, double* d_points_y, double* d_points_z,
  ...
  double max_allowable_force, bool debug, cudaStream_t stream) 
{
  return goalobstacle_heuristic::launch_kernel(
    d_points_x, d_points_y, d_points_z,
    num_points, nn_index, agent_position, agent_velocity, goal_position,
