#include <ga_circular_fields_planner/GoalHeuristicForce.hpp>
...
namespace ga_circular_fields_planner
{
    ...
    GoalHeuristicForce::GoalHeuristicForce(const sackmesser::Interface::Ptr &interface, const std::string &name) : interface_(interface)
    {
        config_ = interface->getConfigurations()->load<Configuration>(name + "/goal_heuristic_force/");
    }

    GoalHeuristicForce::~GoalHeuristicForce() = default;

    gafro::Wrench<double> GoalHeuristicForce::computeForce(const Agent::State &state,  //
                                                           const gafro::Motor<double> &target) const
    {
        ...
        if (interface_->getCallbacks()->execute("get_goal_heuristic_force", response, request))
        {
    ...
    bool GoalHeuristicForce::Configuration::load(const std::string &ns, const std::shared_ptr<sackmesser::Configurations> &server)
    {
...
REGISTER_CLASS(ga_circular_fields_planner::Force, ga_circular_fields_planner::GoalHeuristicForce, "goal_heuristic_force")