...
#include <ga_circular_fields_planner/ObstacleHeuristicForce.hpp>
#include <sackmesser/Callbacks.hpp>

namespace ga_circular_fields_planner
{

    ObstacleHeuristicForce::ObstacleHeuristicForce(const sackmesser::Interface::Ptr &interface, const std::string &name) : interface_(interface)
    {
        config_ = interface->getConfigurations()->load<Configuration>(name + "/obstacle_heuristic_force/");
    }

    ObstacleHeuristicForce::~ObstacleHeuristicForce() = default;

    gafro::Wrench<double> ObstacleHeuristicForce::computeForce(const Agent::State &state,  //
                                                               const gafro::Motor<double> &target) const
    {
        ...
        if (interface_->getCallbacks()->execute("get_obstacle_heuristic_force", response, request))
        {
    ...
    bool ObstacleHeuristicForce::Configuration::load(const std::string &ns, const std::shared_ptr<sackmesser::Configurations> &server)
    {
...
REGISTER_CLASS(ga_circular_fields_planner::Force, ga_circular_fields_planner::ObstacleHeuristicForce, "obstacle_heuristic_force")