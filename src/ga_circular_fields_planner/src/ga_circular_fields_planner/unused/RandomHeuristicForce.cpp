#include <ga_circular_fields_planner/RandomHeuristicForce.hpp>
...
namespace ga_circular_fields_planner
{
    ...
    RandomHeuristicForce::RandomHeuristicForce(const sackmesser::Interface::Ptr &interface, const std::string &name) : interface_(interface)
    {
        config_ = interface->getConfigurations()->load<Configuration>(name + "/random_heuristic_force/");
    }

    RandomHeuristicForce::~RandomHeuristicForce() = default;

    gafro::Wrench<double> RandomHeuristicForce::computeForce(const Agent::State &state,  //
                                                           const gafro::Motor<double> &target) const
    {
        ...
        if (interface_->getCallbacks()->execute("get_random_heuristic_force", response, request))
        {
    ...
    bool RandomHeuristicForce::Configuration::load(const std::string &ns, const std::shared_ptr<sackmesser::Configurations> &server)
    {
...
REGISTER_CLASS(ga_circular_fields_planner::Force, ga_circular_fields_planner::RandomHeuristicForce, "random_heuristic_force")