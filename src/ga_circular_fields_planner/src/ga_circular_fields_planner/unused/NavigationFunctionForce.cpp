...
#include <ga_circular_fields_planner/NavigationFunctionForce.hpp>
#include <sackmesser/Callbacks.hpp>

namespace ga_circular_fields_planner
{

    NavigationFunctionForce::NavigationFunctionForce(const sackmesser::Interface::Ptr &interface, const std::string &name) : interface_(interface)
    {
        config_ = interface->getConfigurations()->load<Configuration>(name + "/navigation_function_force/");
    }

    NavigationFunctionForce::~NavigationFunctionForce() = default;

    gafro::Wrench<double> NavigationFunctionForce::computeForce(const Agent::State &state,  //
                                                               const gafro::Motor<double> &target) const
    {
        ...
        if (interface_->getCallbacks()->execute("get_navigation_function_force", response, request))
        {
    ...
    bool NavigationFunctionForce::Configuration::load(const std::string &ns, const std::shared_ptr<sackmesser::Configurations> &server)
    {
...
REGISTER_CLASS(ga_circular_fields_planner::Force, ga_circular_fields_planner::NavigationFunctionForce, "navigation_function_force")