...
def run_workload(workload_path, plan_name, duration, enable_metrics):
    cmd = [
    ...
    workload_log_dir = eval_dir / 'results' / plan_name / 'logs' / workload_name
...
def run_plan(args, plan_dir):
    ...
    with tqdm(total=len(workloads), desc="Running workloads", position=0, leave=True, file=sys.stderr) as pbar:
        for i, workload in enumerate(workloads):
            ...
            success = run_workload(workload, plan_name, duration, args.enable_metrics)
            if success:
                num_successful_workloads += 1
            successful_workloads.append(success)
    ...
    return 0, (num_successful_workloads, successful_workloads, workloads)
...
def main():
    ...
    try:
        exit_code, ret = run_plan(args, plan_dir)
        if exit_code == 0:
            num_successful_workloads, successful_workloads, workloads = ret
            logging.info(f"Plan execution completed. Successful workloads: {num_successful_workloads}/{len(workloads)}")
            ...
            logging.error(f"Plan execution failed. Exiting.")
            return 1
    except Exception as e:
        logging.error(f"Error executing plan: {str(e)}", exc_info=True)
        return 1

    return 0 if num_successful_workloads == len(workloads) else 1
