...
def _generate_scene_params():
	...
	scenes[0] = cluttered1k
	scenes[1] = cluttered5k
	scenes[2] = cluttered10k
	scenes[3] = narrow_passage
	scenes[4] = narrow_passage2
	return scenes
...
def _generate_procedurally():
	...
	workload_id = 241 # change this
	scenes = _generate_scene_params()
	cpu_options = {0: False, 1: True}
	planning_windows = {0: 50, 1: 100, 2: 150}
	poses = _generate_poses()
	
	k_force = [5.0e-2, 5.0e-2, 5.0e-2, 2.5e-3, 1.0e-3]

	# for i, scene_params in scenes.items(): # change this
	i = 4
	scene_params = scenes[i]
...
def _generate_workload_yaml(params: Dict):
	...
	percept_config = PerceptConfig(
		...
		fields_config=FieldsConfig(
			...
			potential_detect_shell_rad=0.75
		),
